...
class TileDeck(ABC):
    ...
    @classmethod
    def create(cls, deck_type):
        if deck_type in tile_deck_types:
            return tile_deck_types[deck_type]()
        else:
            raise ValueError(f"Invalid deck type: {deck_type}. "
                             f"Valid types are: {tile_deck_types.keys()}")
    ...
    def _initialize_deck(self) -> None:
        ...
        required_attrs = ['image_path', 'metadata_path',
                          'image_rows', 'image_cols', 'start_row']
        for attr in required_attrs:
            if not hasattr(self, attr):
                raise NotImplementedError(
                    f"The attribute {attr} must be set in the derived class.")
    ...
    def _add_tile(self, tile_image, metadata) -> None:
        tile = Tile(tile_image, metadata['name'],
                    metadata['exits'], type(self).__name__)
...
def register_tile_deck(deck_type) -> TileDeck:
    def inner(cls):
        tile_deck_types[deck_type] = cls
        return cls
    return inner


@register_tile_deck('Indoor')
class IndoorTiles(TileDeck):
    def __init__(self):
        super().__init__()
        self.image_path = "assets/tiles.jpg"
        self.metadata_path = "assets/indoor_tiles.json"
        self.start_row = 2
        self._initialize_deck()


@register_tile_deck('Outdoor')
class OutdoorTiles(TileDeck):
    def __init__(self):
        super().__init__()
        self.image_path = "assets/tiles.jpg"
        self.metadata_path = "assets/outdoor_tiles.json"
        self.start_row = 0
        self._initialize_deck()

# Add more tile decks here...
...
if __name__ == '__main__':
    # Create a deck of indoor tiles
    indoor_deck = TileDeck.create('Indoor')
    print(f"{indoor_deck.image_rows} ")
    # Create a deck of outdoor tiles
    outdoor_deck = TileDeck.create('Outdoor')
    ...
    print(f"Is indoor tile: {indoor_tile.tile_type == 'IndoorTiles'}")
    print(f"Is outdoor tile: {indoor_tile.tile_type == 'OutdoorTiles'}")
